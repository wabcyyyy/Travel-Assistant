...
class CriticResult:
    ...
    def as_dict(self) -> dict:
        return {"score": round(self.score, 4), "issues": list(self.issues),
                "strengths": list(self.strengths), "dimensions": dict(self.dimensions)}
...
def critique_plans(plans: list[dict], preferences: list[str] | None = None) -> CriticResult:
    ...
    active_days = [plan for plan in plans if any(
        item.get("item_type") in ("attraction", "food") for item in plan.get("items") or []
    )]
    ...
    for plan in plans:
        ...
        if count == 1 and len(items) == 1:
            score -= 0.05
        day_text = " ".join(
            " ".join(str(item.get(key) or "") for key in ("poi_name", "tag", "remark"))
            for item in items
        ).lower()
    ...
    dimensions = {"pace": sum(day_scores) / len(day_scores), "diversity": diversity,
                  "preference_alignment": preference_score, "completeness": completeness}
