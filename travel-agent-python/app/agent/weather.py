...
职责与口径：
- `get_weather_forecast(city, start_date, end_date)`：城市→坐标（tools.city_center，
...
def get_weather_forecast(city: str, start_date: str | None, end_date: str | None) -> list[dict[str, Any]] | None:
    ...
    if window_start > window_end:
        return None
    # 懒导入：城市坐标链路（city_geo 字典 → Nominatim 兜底）在 tools
    from app.agent import tools

    center = tools.city_center(str(city or "").strip())
    if not center:
        return None
    params = {
