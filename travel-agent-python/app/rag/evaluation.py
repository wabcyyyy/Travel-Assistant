...
def evaluate_retrieval_case(rows: list[dict[str, Any]], case: dict[str, Any]) -> dict[str, Any]:
    ...
    city_accuracy = (not rows or all(row.get("city") == city for row in rows)) if city else True
    category_accuracy = (not rows or all(row.get("category") == category for row in rows)) if category else True
    return {
        ...
        "city_filter_accuracy": float(city_accuracy),
        "category_filter_accuracy": float(category_accuracy),
        ...
        "poi_authority_rate": round(
            sum(str(row.get("source", "")).startswith("mysql") for row in filtered_rows)
            / len(filtered_rows), 4
        ) if filtered_rows else 1.0,
    }
