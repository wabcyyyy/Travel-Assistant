...
def day_done(itinerary_id: int, day_no: int, theme: str | None,
             item_count: int, note: str | None) -> None:
    publish_event(itinerary_id, "day_done", {
        "dayNo": int(day_no), "theme": theme, "itemCount": int(item_count), "note": note,
    })
...
def degraded(itinerary_id: int, scope: str, reason: str | None, fallback: str) -> None:
    ...
    publish_event(itinerary_id, "degraded", {
        "scope": scope, "reason": reason, "fallback": fallback,
    })


def error(itinerary_id: int, code: str, message: str | None, retryable: bool) -> None:
    publish_event(itinerary_id, "error", {
        "code": code, "message": message, "retryable": bool(retryable),
    })


def complete(itinerary_id: int, status: str, day_count: int | None,
             degraded_days: list[int], version_id: int | None) -> None:
    """终态事件：前端据此停止监听。`status` 只有 COMPLETED / PARTIAL 两值。"""
    publish_event(itinerary_id, "complete", {
        "status": status,
        "dayCount": day_count,
...
def export_done(itinerary_id: int, task_id: int, status: str, download_url: str) -> None:
    ...
    publish_event(itinerary_id, "export_done", {
        "taskId": int(task_id), "status": status, "downloadUrl": download_url,
    })
