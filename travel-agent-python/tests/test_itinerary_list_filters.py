    ...
    with db_session.session_scope() as session:
        for title, city, gen_state, favorite, archived in rows:
            session.add(ItineraryMain(
                user_id=42, title=title, city=city, days=1, persons=1,
                gen_state=gen_state, favorite=favorite, archived=archived,
            ))
...
def test_favorite_toggle(client: TestClient) -> None:
    ...
    response = client.post(
        f"/api/itinerary/{trip_id}/favorite", json={"favorite": True}, headers=_headers()
    )
...
def test_archive_toggle_moves_row_out_of_default_view(client: TestClient) -> None:
    ...
    trip_id = _row_id(client, "北京 2 日 · 生成中")
    response = client.post(
        f"/api/itinerary/{trip_id}/archive", json={"archived": True}, headers=_headers()
    )
    assert response.status_code == 200
