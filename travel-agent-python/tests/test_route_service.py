...
def test_route_service_uses_provider_and_cache(monkeypatch):
    ...
    def fetcher(first, second, mode, departure):
        ...
        return {
            "from_poi_id": first["poi_id"], "to_poi_id": second["poi_id"],
            "mode": mode, "distance_m": 1000, "duration_min": 12,
            "source": "amap", "confidence": 0.95, "degraded": False,
        }
...
def test_route_service_peak_factor_is_explicit(monkeypatch):
    ...
    clear_route_cache()
    service = RouteService(fetcher=lambda *_args: {
        "from_poi_id": "A", "to_poi_id": "B", "mode": "walking",
        "distance_m": 1000, "duration_min": 10, "source": "amap",
