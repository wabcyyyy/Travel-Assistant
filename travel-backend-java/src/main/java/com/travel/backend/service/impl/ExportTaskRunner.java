...
public class ExportTaskRunner {
    ...
    private Map<String, Object> buildModel(ItineraryMain main) {
        ...
        for (ItineraryDay day : days) {
            ...
            for (ItineraryItem item : itemEntities) {
                ...
                // 类型/来源做用户化映射，不在印刷品上外露内部编码（与站内口径一致）
                itemMap.put("typeLabel", typeLabel(item.getItemType()));
                itemMap.put("srcLabel", sourceLabel(item.getSource()));
    ...
    private String sourceLabel(String source) {
        if (source == null) return "行程设定";
        return switch (source) {
            case "client-context" -> "行程设定";
            case "mysql.poi_knowledge" -> "目的地知识库";
            case "llm.open_day" -> "开放研究";
            case "amap-grounding", "amap" -> "高德地图";
            default -> "AI 生成";
        };
    }
