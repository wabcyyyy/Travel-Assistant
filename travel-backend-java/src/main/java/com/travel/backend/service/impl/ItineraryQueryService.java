...
public class ItineraryQueryService {
    ...
    private static final Logger log = LoggerFactory.getLogger(ItineraryQueryService.class);
    ...
    public ItineraryVO detail(Long userId, Long id) {
        ...
        // 住宿晚数以主表落库值为准（生成时按 N 天 = N-1 晚派生）；
        // 不能按「有酒店项的天数」反推——最后一晚退房日没有酒店项，会导致 3 天行程显示 1 晚。
        vo.setStayNights(main.getStayNights() != null
                ? main.getStayNights() : Math.max(dayVOList.size() - 1, 0));
    ...
    private Map<String, Object> parseFactEvidence(String json) {
        ...
        try {
            ...
            log.warn("json column parse failed, returned null instead", ignored);
            return null;
        }
    }
