<template>
  <div class="dashboard">
    <p class="dash-date lp-label">{{ todayLabel }} · 今天从这里开始</p>
    ...
    <div v-else-if="summaries.length" class="dash-grid">
      ...
      <div class="dash-main">
        ...
        <!-- 统计带：一条横切，发丝分隔，不是四张同款卡 -->
        <div class="metrics-strip">
          <div class="metric">
            <span class="man-k">到访国家</span>
            <p class="metric-v">{{ atlas?.stats.countryCount ?? '—' }}<span class="man-u">国</span></p>
          </div>
          <div class="metric">
            <span class="man-k">行程</span>
            <p class="metric-v">{{ summaries.length }}<span class="man-u">段</span></p>
          </div>
          <div class="metric">
            <span class="man-k">旅行天数</span>
            <p class="metric-v">{{ totalDays }}<span class="man-u">天</span></p>
          </div>
          <div class="metric">
            <span class="man-k">已点亮城市</span>
            <p class="metric-v">{{ atlas?.stats.cityCount ?? '—' }}<span class="man-u">城</span></p>
          </div>
        </div>
...
.hero-badge {
  ...
  align-items: center;
  padding: 7px 14px;
  border: 1px solid color-mix(in srgb, var(--lp-text-inverse) 24%, transparent);
  border-radius: var(--lp-radius-pill);
  background: color-mix(in srgb, var(--lp-bg-inverse) 55%, transparent);
  backdrop-filter: blur(18px) saturate(1.5);
  -webkit-backdrop-filter: blur(18px) saturate(1.5);
  box-shadow: inset 0 1px 0 color-mix(in srgb, var(--lp-text-inverse) 12%, transparent);
...
.hero-ticket {
  ...
  background: color-mix(in srgb, var(--lp-ticket-bg) 88%, transparent);
  backdrop-filter: blur(20px) saturate(1.4);
  -webkit-backdrop-filter: blur(20px) saturate(1.4);
  box-shadow:
    var(--lp-shadow-sm),
    var(--lp-shadow-lg),
    inset 0 1px 0 color-mix(in srgb, var(--lp-text-inverse) 18%, transparent);
